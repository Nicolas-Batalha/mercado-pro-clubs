// =========================================================================
// MERCADO PRO CLUBS - MOTOR DO MERCADO DE TRANSFERÊNCIAS
// =========================================================================
...
// ── EXPIRAÇÃO: 5 horas em milissegundos ──────────────
const TEMPO_EXPIRACAO = 5 * 60 * 60 * 1000;
...
function calcularTempoDecorrido(tempoCriacao) {
    if (!tempoCriacao) return "Agora mesmo";
    const min = Math.floor((Date.now() - tempoCriacao) / 60000);
    if (min < 1)  return "Agora mesmo";
    if (min < 60) return `Há ${min} min`;
    return `Há ${Math.floor(min / 60)} hora(s)`;
}

function calcularTempoRestante(criadoEm) {
    if (!criadoEm) return '';
    const restante = TEMPO_EXPIRACAO - (Date.now() - criadoEm);
    if (restante <= 0) return 'Expirando...';
    const horas   = Math.floor(restante / 3600000);
    const minutos = Math.floor((restante % 3600000) / 60000);
    if (horas > 0) return `Expira em ${horas}h ${minutos}min`;
    return `Expira em ${minutos}min`;
}

// --- BUSCAR VAGAS E LIMPAR EXPIRADAS ---
    ...
    try {
        ...
        snapshot.forEach(d => {
            ...
            if (vaga.criadoEm && (agora - vaga.criadoEm) > TEMPO_EXPIRACAO) {
                deletarPromises.push(deleteDoc(doc(db, "vagas", d.id)));
            } else {
                todasAsVagas.push(vaga);
            }
        });

        if (deletarPromises.length > 0) {
...
// --- RESPONDER PEDIDO ---
// FIX #1: após aceitar, recarrega o painel para mostrar o botão de chat
...
function renderizarFeed(vagas) {
    ...
    feedLfg.innerHTML = vagas.map(vaga => {
        ...
        const dono    = vaga.criadorUid || '';
        const ehDono  = usuarioLogado && usuarioLogado.uid === dono;
        ...
        <div class="lfg-card">
            ...
            <div class="card-rodape">
                ...
                <button class="btn-chamar" data-capitao="${dono}" data-clube="${vaga.clube || ''}">
...
if (formLfg) {
    formLfg.addEventListener('submit', async (e) => {
        ...
        if (!usuario) {
            mostrarToast("Faça login para publicar uma vaga!", "erro");
            return;
        }
...
// --- PAINEL DO CAPITÃO ---
// FIX #1: pedidos pendentes mostram aceitar/recusar; pedidos aceitos mostram botão de chat
async function carregarSolicitacoesDeTeste(capitaoUid) {
    try {
        const container = document.getElementById('painel-notificacoes');
        if (!container) return;

        // Busca apenas pedidos pendentes
        const snapPendentes = await getDocs(query(
        ...
        snapPendentes.forEach(docSnap => {
            const p = docSnap.data();
            container.innerHTML += `
            <div style="background:#1a1a1a;border:1px solid #333;border-radius:8px;padding:16px;margin-bottom:12px;">
                <div style="display:flex;align-items:center;gap:16px;margin-bottom:12px;">
                    <div style="background:#12E06C;color:#000;font-size:22px;font-weight:900;width:50px;height:50px;display:flex;align-items:center;justify-content:center;border-radius:8px;">${p.jogadorOverall}</div>
                    <div>
